...
int player_1_score;
int player_2_score;
char squares[3][3] = { '*', '*', '*', '*', '*', '*', '*', '*', '*' };	// 3 Collums and 3 rows

void main_menu()
{
	system("cls");	// To clear the screen (e.g = the residue of previous texts)
...
int main()
{
	// Now
		...
		else if (input == "s")
		{
			system("cls");
			n = 0;
			draw();	// If you put the draw(); below the while(1), it will flicker
			while (1)
			{
				n++;
				player_input();
